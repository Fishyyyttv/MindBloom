...
import { Plus, X, Save, Trash2, Shuffle, MessageCircle } from 'lucide-react'
...
export default function DiaryPage() {
  ...
  useEffect(() => {
    ...
    const loadEntries = async () => {
      ...
      try {
        ...
        toast.error('Could not load journal entries right now')
      } finally {
  ...
  const buildChatPrompt = () => {
    const parts = ['I want to talk about this journal entry draft.']

    if (title.trim()) {
      parts.push(`Title: ${title.trim()}`)
    }

    parts.push(`Entry: ${content.trim() || '(empty draft)'}`)

    if (mood) {
      parts.push(`Mood (1-5): ${mood}`)
    }

    parts.push('Help me process this in a supportive way.')
    return parts.join('\n')
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
                ...
                {MOOD_EMOJIS.map((emoji, index) => (
                  <button
                    ...
                    className={cn(
                      ...
                      mood === index + 1 ? 'bg-sage-100 scale-110' : 'hover:bg-sage-50'
                    )}
                  >
                    {emoji}
                  </button>
            ...
            <div className="flex gap-3">
              <Link
                ...
                className={cn(
                  ...
                  !content.trim() && 'pointer-events-none opacity-50'
                )}
              >
      ...
      {loadingEntries && (
        <p className="text-xs text-muted">Loading entries...</p>
      )}

      {entries.length === 0 && !composing && !loadingEntries && (
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between p-5 border-b border-sage-100 shrink-0">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => deleteEntry(viewEntry.id)}
