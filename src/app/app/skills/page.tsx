                ...
                {activeSkill.steps.map((step, i) => (
                  <button
                    ...
                    className={cn(
                      ...
                      checkedSteps.has(i) ? 'bg-sage-50 border border-sage-200' : 'bg-gray-50 border border-transparent hover:border-sage-100'
                    )}
                  >
                    <CheckCircle2 className={cn('w-4 h-4 mt-0.5 shrink-0 transition-colors', checkedSteps.has(i) ? 'text-sage-500' : 'text-gray-300')} />
                    <span className={cn('text-sm leading-relaxed', checkedSteps.has(i) ? 'text-sage-700' : 'text-charcoal')}>{step}</span>
                  </button>
                ))}
              </div>

              {checkedSteps.size === activeSkill.steps.length && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4 bg-sage-50 border border-sage-200 rounded-xl p-4 text-center">
