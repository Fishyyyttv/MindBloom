...
export default function WorksheetsPage() {
  ...
  const save = async () => {
    ...
    setSaving(true)
    await supabase.from('worksheet_completions').insert({
      user_id: user.id,
      worksheet_type: activeId,
      responses,
    })
    setSaving(false)
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted text-sm">Guided reflection to help you make sense of what you're feeling</p>
      </div>

      <div className="relative">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted" />
        <input value={search} onChange={e => setSearch(e.target.value)}
          placeholder="Search worksheets..." className="input pl-9" />
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {filtered.map((w, i) => (
          <motion.button
            ...
            onClick={() => { setActiveId(w.id); setResponses({}) }}
          ...
          >
            ...
            <div className="flex items-start justify-between gap-2">
              <h3 className="font-semibold text-charcoal group-hover:text-sage-700 transition-colors">{w.name}</h3>
              <span className="badge bg-sage-50 text-sage-700 border border-sage-100 shrink-0 text-xs">{w.category}</span>
            </div>
            <p className="text-sm text-muted mt-1 leading-relaxed">{w.description}</p>
          </motion.button>
        ))}
      </div>
        ...
        {activeId && activeWorksheet && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 bg-black/30 z-50" onClick={() => setActiveId(null)} />
            <motion.div
                ...
                {questions.map((q, i) => (
                  <div key={i}>
                    ...
                    <textarea
                      ...
                      onChange={e => setResponses(prev => ({ ...prev, [q.label]: e.target.value }))}
